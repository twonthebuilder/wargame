...
global.localStorage = (() => {
  ...
  const store = new Map();
  const storedBytes = () =>
    Array.from(store, ([key, value]) => (key.length + value.length) * 2).reduce(
      (total, entryBytes) => total + entryBytes,
      0
    );
  return {
    ...
    setItem: (key, value) => {
      ...
      const previousBytes =
        previousValue === undefined ? 0 : (normalizedKey.length + previousValue.length) * 2;
...
async function runTests() {
  ...
  assert.strictEqual(
    safeAdapter.getItem('missing'),
    null,
    'safe adapter should treat inaccessible storage as empty'
  );
  assert.deepStrictEqual(
    safeAdapter.keys(),
    [],
    'safe adapter should report no keys when storage is inaccessible'
  );
  ...
  const notificationStack = {
    ...
    visible: new Map([
      [
        'v1',
        { item: { id: 'live', title: 'Live', lines: ['Active'], duration: 1500, tone: 'warning' } },
      ],
    ]),
  };
  ...
  assert.strictEqual(
    snap.mandates.currentTick,
    mandateSnapshot.currentTick,
    'mandate state should persist'
  );
  ...
  assert.strictEqual(
    snap.tutorial.frontierSweep.targetTileKey,
    '0,0',
    'tutorial state should persist'
  );
  ...
  assert.strictEqual(
    difficultyAlignment.state.difficulty,
    5,
    'difficulty should reconcile to the saved wars-won value'
  );
  assert.strictEqual(
    difficultyAlignment.stats.warsWon,
    5,
    'warsWon should remain the authoritative progress value'
  );
  ...
  assert.strictEqual(
    lowerWarsWon.state.difficulty,
    1,
    'difficulty should follow stats.warsWon when provided'
  );
  assert.strictEqual(
    lowerWarsWon.stats.warsWon,
    1,
    'warsWon should not be overridden by difficulty'
  );
  ...
  assert.strictEqual(
    missingWarsWon.stats.warsWon,
    3,
    'missing warsWon should inherit the current difficulty'
  );
  ...
  assert.strictEqual(
    result.factionState.standings.reformers,
    50,
    'missing faction standings should default'
  );
  assert.strictEqual(
    result.tutorial.frontierSweep.targetTileKey,
    '2,0',
    'tutorial state should hydrate'
  );
  assert.strictEqual(
    result.tutorial.frontierSweep.spreadImmune,
    false,
    'tutorial spread immunity should hydrate'
  );
  assert.ok(
  ...
  const sanitized = Persistence.deserializeGameState(malformedSnapshot, {
    hexFactory: (q, r, s) => new Hex(q, r, s),
  });
  assert.strictEqual(sanitized.overworld.hexes.size, 3, 'invalid hex entries should be skipped');
  const sanitizedTypes = Array.from(sanitized.overworld.hexes.values())
    .map((t) => t.type)
    .sort();
  assert.deepStrictEqual(
    sanitizedTypes,
    ['castle', 'field', 'shrine'],
    'only valid tile ids should survive'
  );
  assert.strictEqual(
    sanitized.overworld.hexes.get('2,0').owner,
    null,
    'unknown owners should be coerced to null'
  );
  assert.strictEqual(
    sanitized.overworld.hexes.get('3,0').owner,
    'rebel',
    'recognized owners should be normalized to lowercase'
  );

  const legacyRebelSnapshot = {
    overworld: {
      hexes: [{ q: 4, r: 0, s: -4, type: 'rebel', owner: 'rebel' }],
    },
    stats: {},
  };
  ...
  assert.strictEqual(
    legacyRebelTile.type,
    'rebelcamp',
    'legacy rebel tiles should normalize to rebel camps'
  );
  ...
  const legacyRestore = Persistence.deserializeGameState(legacySnapshot, {
    hexFactory: (q, r, s) => new Hex(q, r, s),
  });
  assert.strictEqual(
    legacyRestore.timekeeper.ticks,
    START_TICK,
    'missing timekeeper ticks should align to the default start tick'
  );
  assert.strictEqual(
    legacyRestore.timekeeper.daysPerWeek,
    7,
    'legacy saves should default to seven-day weeks'
  );
  assert.strictEqual(
    legacyRestore.timekeeper.weeksPerMonth,
    4,
    'legacy saves should default to four-week months'
  );
  assert.strictEqual(
    legacyRestore.narrative,
    null,
    'legacy saves should default to empty narrative state'
  );
  ...
  assert.deepStrictEqual(
    restoredCalendar,
    defaultCalendar,
    'restored calendar math should match Timekeeper defaults'
  );
  ...
  const altGame = {
    ...
    gold: 999,
    imperialFavor: 12,
    stats: { totalKills: 42, bestLevel: 7, warsWon: 5, warsFought: 12 },
  };
  ...
  assert.strictEqual(
    favorSave.payload.imperialFavor,
    2,
    'saves should capture the latest imperial favor value'
  );
  ...
  const reloadedPenalty = Persistence.loadSnapshot(5, {
    hexFactory: (q, r, s) => new Hex(q, r, s),
  });
  ...
  const exoticTypes = Array.from(exoticReload.state.overworld.hexes.values())
    .map((t) => t.type)
    .sort();
  ...
  assert.deepStrictEqual(
    exoticIncome,
    { gold: 4, wood: 0 },
    'income should honor saved mine/ruin data'
  );
  ...
  assert.strictEqual(
    failureSave.error,
    'quota-exceeded',
    'quota errors should be labeled for UI messaging'
  );
  assert.ok(
    failureSave.payload.stats.lastSaveISO,
    'even failed saves should timestamp the payload for status displays'
  );
  ...
  const remoteLoad = Persistence.loadSnapshot('cloud', {
    hexFactory: (q, r, s) => new Hex(q, r, s),
  });
